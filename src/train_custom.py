...
def parse_args():
    ...
    p.add_argument("--weights", type=str, default="yolov8n.pt",
                   help="initial weights (yolov8n.pt recommended for quick tests)")
...
def choose_device(user_device: str = "") -> str:
    ...
    """
    if user_device:  # explicit override from CLI
        return user_device
...
def check_files(args):
    ...
    base_dir = data_path.parent  # e.g. 'data/'
    ...
    for key in ("train", "val"):
        ...
        raw = cfg[key]
        p = Path(raw)
        # Resolve relative paths with respect to data.yaml location
        if not p.is_absolute():
            p = base_dir / p

        if not p.exists():
            print(f"[ERROR] Path specified in data.yaml does not exist:")
            print(f"  {key}: {raw}")
            print(f"  resolved as: {p.resolve()}")
            sys.exit(1)
...
def main():
    ...
    # Decide device (same behavior as webcam script)
    ...
    print(f"  device:  {chosen_device} (CLI: {'auto' if args.device == '' else args.device})")
    print()
    ...
    try:
        model.train(
            ...
            name=args.name,
            device=chosen_device,   # our resolved device
    ...
    print("[INFO] Training finished. Check the runs directory for outputs "
